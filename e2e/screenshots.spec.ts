...
test('note history panel', async ({ page }) => {
  ...
  const historyPreview = historyPanel.locator('.note-history-preview')
  const [restoreBox, previewBox] = await Promise.all([restoreButton.boundingBox(), historyPreview.boundingBox()])
  expect(restoreBox).not.toBeNull()
  expect(previewBox).not.toBeNull()
  expect(restoreBox!.width).toBeLessThan(previewBox!.width * 0.6)
  await settleLayout(page)
