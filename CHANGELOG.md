...
### Fixed
- Polished the screenshot-reviewed workspace surfaces: docked Git changes no longer clip row actions, companion panels no longer cover footer/repository state, Preview QA labels have readable hierarchy, the Inspector no longer mislabels unresolved citations as total citations, Knowledge repair and Note History use intentional empty/preview states, onboarding is fully opaque with visible progress, plugin navigation stays on one row, Vault Health keeps its 3×3 diagnostic matrix, inspector cards no longer jump on hover, and dark screenshot/E2E workspaces now let the editor follow the app theme instead of pinning a white Monaco surface.
