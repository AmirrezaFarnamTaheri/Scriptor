...
fn rewrite_hashtags_outside_code_and_urls(
    ...
    while let Some(pos) = rest.find('`') {
        ...
        if in_code {
            ...
            out.push_str(&rewrite_non_url_segment(segment, old_root, new_root, edits));
        }
        out.push('`');
        rest = &tail[1..];
        in_code = !in_code;
    ...
    if in_code {
        ...
        out.push_str(&rewrite_non_url_segment(rest, old_root, new_root, edits));
    }
    out
}

fn rewrite_non_url_segment(
    segment: &str,
    old_root: &str,
    new_root: &str,
    edits: &mut u32,
    ...
    segment
        ...
        .map(|token| {
            ...
            if core.contains("://") {
                ...
                format!(
                    "{}{}",
                    rewrite_hashtag_line(core, old_root, new_root, edits),
                    suffix
                )
            }
        })
