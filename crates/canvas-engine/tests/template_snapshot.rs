...
use scriptor_canvas_engine::{
    SnapshotFormat, apply_template, apply_template_dry_run, load_document_file, render_svg,
    write_snapshot,
};
...
fn template_apply_writes_undo_checkpoint_and_snapshot() {
    ...
    let snapshot =
        write_snapshot(&applied.document, &output, SnapshotFormat::Svg, false).expect("snapshot");
