...
fn with_verified_vault<T>(
    ...
    let _switch = match state.vault_switch_lock.try_lock() {
        ...
        Err(TryLockError::Poisoned(poisoned)) => {
            tracing::error!(
                lock = "vault switch",
                "recovering poisoned vault-switch lock"
            );
...
#[tauri::command]
pub fn daemon_reload_config(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_reload_config(&state)
}
#[tauri::command]
pub fn daemon_health_diagnostics(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_health_diagnostics(&state)
}
#[tauri::command]
pub fn daemon_health_report(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_health_report(&state)
}
#[tauri::command]
pub fn daemon_rebuild_index(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_rebuild_index(&state)
}
#[tauri::command]
pub fn daemon_search(
    ...
    bridge_search(&state, query, limit)
}
#[tauri::command]
pub fn daemon_list_note_summaries(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_list_note_summaries(&state)
}
#[tauri::command]
pub fn daemon_backlinks(state: tauri::State<AppState>, path: String) -> Result<String, String> {
    bridge_backlinks(&state, path)
}
#[tauri::command]
pub fn daemon_graph(
    state: tauri::State<AppState>,
    ...
    bridge_graph(&state, focus_path, depth)
}
#[tauri::command]
pub fn daemon_git_status(state: tauri::State<AppState>) -> Result<String, String> {
    bridge_git_status(&state)
}
#[tauri::command]
pub fn daemon_save_note(
    ...
    bridge_save_note(&state, path, markdown, expected_content_hash, dry_run)
}
#[tauri::command]
pub fn daemon_update_note_index(
    state: tauri::State<AppState>,
    path: String,
) -> Result<bool, String> {
    bridge_update_note_index(&state, path)
}
#[tauri::command]
...
pub(crate) fn bridge_reload_config(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon reload config", reload_config_impl)
}
pub(crate) fn bridge_rebuild_index(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon rebuild index", rebuild_index_impl)
}
pub(crate) fn bridge_update_note_index(state: &AppState, path: String) -> Result<bool, String> {
    with_verified_vault(state, "daemon update note index", || {
        update_note_index_impl(path)
    })
}
pub(crate) fn bridge_search(state: &AppState, query: String, limit: u32) -> Result<String, String> {
    with_verified_vault(state, "daemon search", || search_impl(query, limit))
}
pub(crate) fn bridge_list_note_summaries(state: &AppState) -> Result<String, String> {
    with_verified_vault(
        state,
        "daemon list note summaries",
        list_note_summaries_impl,
    )
}
pub(crate) fn bridge_backlinks(state: &AppState, path: String) -> Result<String, String> {
    with_verified_vault(state, "daemon backlinks", || backlinks_impl(path))
}
pub(crate) fn bridge_graph(
    state: &AppState,
    focus_path: Option<String>,
    depth: u32,
) -> Result<String, String> {
    with_verified_vault(state, "daemon graph", || graph_impl(focus_path, depth))
}
pub(crate) fn bridge_git_status(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon git status", git_status_impl)
}
pub(crate) fn bridge_save_note(
    ...
    with_verified_vault(state, "daemon save note", || {
        save_note_impl(path, markdown, expected_content_hash, dry_run)
    })
}
pub(crate) fn bridge_rename_apply(
    ...
    with_verified_vault(state, "daemon rename", || {
        rename_apply_impl(from_path, to_path, update_links, expected_source_hash)
    })
}
pub(crate) fn bridge_health_report(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon health report", health_report_impl)
}
pub(crate) fn bridge_health_diagnostics(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon health diagnostics", health_diagnostics_impl)
}
pub(crate) fn bridge_export_run_note(
    ...
    with_verified_vault(state, "daemon export note", || {
        export_run_note_impl(
            note_path,
            format,
            dry_run,
            extra_pandoc_args,
...
pub(crate) fn bridge_export_run_markdown(
    ...
    with_verified_vault(state, "daemon export markdown", || {
        export_run_markdown_impl(
            note_path,
            source_markdown,
            format,
            dry_run,
...
pub(crate) fn bridge_export_start_note(
    ...
    with_verified_vault(state, "daemon export start", || {
        export_start_note_impl(
            note_path,
            format,
            dry_run,
            extra_pandoc_args,
...
pub(crate) fn bridge_export_job_status(state: &AppState) -> Result<String, String> {
    with_verified_vault(state, "daemon export status", export_job_status_impl)
}
pub(crate) fn bridge_export_cancel(state: &AppState, job_id: Option<String>) -> Result<(), String> {
    with_verified_vault(state, "daemon export cancel", || export_cancel_impl(job_id))
}
