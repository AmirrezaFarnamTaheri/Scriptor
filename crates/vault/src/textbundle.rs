...
pub fn import_text_bundle(
    ...
    if archive.len() > MAX_TEXTBUNDLE_ENTRIES {
        return Err(VaultError::InvalidConfig {
            message: format!(
                "TextBundle has too many entries: {} > {MAX_TEXTBUNDLE_ENTRIES}",
                archive.len()
            ),
        });
    }
    ...
    for index in 0..archive.len() {
        ...
        total_bytes = total_bytes
            ...
            .ok_or_else(|| VaultError::InvalidConfig {
                message: "TextBundle expanded size overflow".into(),
            })?;
        if total_bytes > MAX_TEXTBUNDLE_TOTAL_BYTES {
            return Err(VaultError::InvalidConfig {
                message: format!("TextBundle expands beyond {MAX_TEXTBUNDLE_TOTAL_BYTES} bytes"),
            });
        }
        if name == "text.md" {
            if size > MAX_TEXTBUNDLE_MARKDOWN_BYTES {
                return Err(VaultError::InvalidConfig {
                    message: "TextBundle text.md is too large".into(),
                });
            }
            continue;
        }
        ...
        if size > MAX_TEXTBUNDLE_ASSET_BYTES {
            return Err(VaultError::InvalidConfig {
                message: format!("TextBundle asset is too large: {name}"),
            });
        }
    ...
    let mut markdown = String::new();
    {
        ...
        if markdown.len() as u64 > MAX_TEXTBUNDLE_MARKDOWN_BYTES {
            return Err(VaultError::InvalidConfig {
                message: "TextBundle text.md exceeded the read budget".into(),
            });
        }
    }
    ...
    let result = (|| -> Result<TextBundleImportOutput, VaultError> {
        ...
        for (index, rel, declared_size) in &asset_plan {
            ...
            let mut bytes =
                Vec::with_capacity((*declared_size).min(MAX_TEXTBUNDLE_ASSET_BYTES) as usize);
            (&mut entry)
                .take(MAX_TEXTBUNDLE_ASSET_BYTES + 1)
                .read_to_end(&mut bytes)
                .map_err(|error| VaultError::InvalidConfig {
                    message: error.to_string(),
                })?;
            if bytes.len() as u64 > MAX_TEXTBUNDLE_ASSET_BYTES {
                return Err(VaultError::InvalidConfig {
                    message: format!("TextBundle asset exceeded read budget: {}", rel.as_str()),
                });
            }
        ...
        for (rel, stage_path) in &staged {
            ...
            if let Err(source) = fs::rename(stage_path, &dest) {
                for prior in promoted.iter().rev() {
        ...
        Ok(TextBundleImportOutput {
            ...
            assets_imported: promoted.len() as u32,
        })
    })();
