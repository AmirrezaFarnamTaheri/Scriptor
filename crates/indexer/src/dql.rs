...
fn extract_regex_after_preserving_case(
    input: &str,
    lower: &str,
    prefix: &str,
) -> Option<String> {
...
fn path_matches(
    ...
    let rows = ensure_candidate_bound(
        rows.collect::<Result<Vec<_>, _>>()?,
        "path matches",
    )?;
    Ok(rows.into_iter().filter(|row| re.is_match(&row.path)).collect())
}
...
fn phrase_search(
    ...
    let rows = stmt.query_map(
        params![fts_expr, vault_id, DQL_CANDIDATE_FETCH],
        |row| {
            Ok(DqlResultRow {
                path: row.get(1)?,
                title: row.get(2)?,
...
fn body_excludes(
    ...
    let matching: std::collections::BTreeSet<String> = search_notes(cache, vault_id, term, DQL_CANDIDATE_FETCH as u32)?
        .into_iter()
        .map(|h| h.path)
        .collect();
