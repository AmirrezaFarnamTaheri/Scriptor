...
pub(crate) fn has_ignored_segment(relative: &str) -> bool {
    relative.split('/').any(|segment| IGNORED_SEGMENTS.contains(&segment))
}
...
fn scan_directory(
    ...
    let walker = WalkDir::new(directory)
        ...
        .filter_entry(|entry| {
            entry.depth() == 0
                || !IGNORED_SEGMENTS.iter().any(|ignored| entry.file_name() == std::ffi::OsStr::new(ignored))
        });
    ...
    for entry in walker {
        ...
        let (content, content_hash) = if should_capture {
            ...
            (Some(String::from_utf8_lossy(&bytes).into_owned()), Some(hash))
        } else {
            (None, None)
        };
