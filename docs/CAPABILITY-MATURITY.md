...
| Tantivy index | Evaluation | `crates/tantivy-indexer/` | Excluded from default workspace build and release binaries; benched against FTS5 (2026-09-01, release build, 2k-note vault): warm search 0.0ms vs FTS5 3.2ms (both far under the 100ms budget), but per-note-commit index build cost 149.5s for 2k notes — disqualifying as a search drop-in while FTS5 warm search meets budget; a batched-commit API would be the next step if search latency ever regresses. Comparable via `cargo run --release -p scriptor-cli --features tantivy -- bench-tantivy <vault> <query>` |
