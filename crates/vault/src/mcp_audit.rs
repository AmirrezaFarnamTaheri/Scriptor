...
pub fn verify_mcp_audit_chain(root: &VaultRoot) -> Result<(), VaultError> {
    ...
    for path in files {
        ...
        for line in BufReader::new(file).lines() {
            ...
            let record: McpMutationAuditRecord =
                serde_json::from_str(&line).map_err(|error| VaultError::InvalidConfig {
                    message: format!("invalid MCP audit record in {}: {error}", path.display()),
                })?;
            if !head_is_anchor && record.previous_hash != previous {
                return Err(VaultError::InvalidConfig {
                    message: format!("MCP audit chain fork/tamper detected at {}", record.id),
                });
            }
            ...
            let mut hasher = Sha256::new();
            if let Some(prev) = previous_for_hash {
            ...
            if computed != claimed {
                return Err(VaultError::InvalidConfig {
                    message: format!("MCP audit record hash mismatch at {}", record.id),
                });
            }
...
mod tests {
    ...
    fn seed_with(
        ...
        read_last_record_hash(&path)?.ok_or_else(|| VaultError::InvalidConfig {
            message: "seeded record missing hash".into(),
        })
    }
    ...
    #[test]
    fn verify_accepts_a_pruned_head_and_rejects_internal_tampering()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        previous = Some(seed_with(
            &root,
            ".scriptor/audit/mcp-mutations-0001.jsonl",
            intent("old"),
            previous,
        )?);
        previous = Some(seed_with(
        ...
        previous = Some(seed_with(
            &root,
            ".scriptor/audit/mcp-mutations-0002.jsonl",
            intent("mid"),
            previous,
        )?);
        previous = Some(seed_with(
