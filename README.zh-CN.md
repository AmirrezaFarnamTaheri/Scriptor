...
[![Version](https://img.shields.io/badge/version-1.2.7-0f766e.svg)](VERSION)
...
生产安装包通过 GitHub Release 发布。当前版本为 **1.2.7**。
...
**积极开发中。** `v1.2.7` 是当前生产候选版本。Desktop、vault、indexer、knowledge、Git、export、daemon 和 Web 界面均已实现并发布。[`docs/CAPABILITY-MATURITY.zh-CN.md`](docs/CAPABILITY-MATURITY.zh-CN.md) 中的能力账本是判断哪些功能受支持、处于实验阶段或仅停留在设计阶段的权威依据。移动端、加密 vault、本地 embeddings、Tantivy 和 WASM host 仍属于实验性或仅设计阶段能力。
