...
test.describe('visual review states', () => {
  ...
  test('Git mutation confirmation evidence', async ({ page }) => {
    ...
    const git = page.locator('.git-panel')
    await expect(git).toBeVisible()
    await expect(git).toContainText('Research Plan.md')
    await settleLayout(page)
    await captureElement(page, git, 'visual-git-status.png')
  ...
  test('note history restore confirmation evidence', async ({ page }) => {
    ...
    await expect(history).toContainText('Compare before restoring')
    await settleLayout(page)
    await captureElement(page, history, 'visual-note-history-comparison.png')
