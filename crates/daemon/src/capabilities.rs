...
pub fn capability_for_command(command: &str) -> Option<&'static str> {
    match command {
        ...
        "export_run_note" | "export_run_markdown" | "export_start_note" | "export_start_markdown" | "export_cancel" => Some("scriptor.export"),
...
pub fn enforce(state: &PluginState, method: &RpcMethod) -> Result<(), RpcError> {
    let Some(capability_id) = capability_for_method(method) else { return Ok(()); };
    if state.is_enabled(capability_id) { Ok(()) } else {
        Err(RpcError::PluginDisabled { capability_id: capability_id.to_string() })
    }
}
...
mod tests {
    ...
    fn disabled_graph_and_export_fail_before_dispatch() {
        let state = PluginState { disabled_plugins: ["scriptor.graph".into(), "scriptor.export".into()].into_iter().collect(), ..Default::default() };
        assert!(matches!(enforce(&state, &RpcMethod::GraphSummary { path: None, depth: 1 }), Err(RpcError::PluginDisabled { capability_id }) if capability_id == "scriptor.graph"));
        assert!(matches!(
