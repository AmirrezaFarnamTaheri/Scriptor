...
Идентичность палитры и светлый/тёмный режим — независимые измерения во время выполнения. Scriptor записывает `data-palette="<id>"` для выбранной палитры, `data-appearance="light|dark"` для фактического режима и `data-theme` как селектор встроенной палитры. Сохранённая настройка `system` следует режиму ОС и во время выполнения преобразуется в `data-appearance`. Переключатель день/ночь в верхней панели меняет только режим и никогда не перебирает и не заменяет выбранную палитру.

### Светлая базовая палитра (`data-theme="light"`, обычно `data-appearance="light"`)
...
### Тёмная базовая палитра (`data-theme="dark"`, обычно `data-appearance="dark"`)
